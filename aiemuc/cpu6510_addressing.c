...
static inline void addr_absx_write(uint8_t value) {
    ...
    WAIT_READY_THEN_READ(base, absx_w_dummy_wait);
    WAIT_READY_THEN_WRITE(cpu.addr_abs, value, absx_w_data_wait);
}
...
static inline void addr_absy_write(uint8_t value) {
    ...
    cpu.addr_abs = base + cpu.y;
    // Always dummy read for writes
    WAIT_READY_THEN_READ(base, absy_w_dummy_wait);
    WAIT_READY_THEN_WRITE(cpu.addr_abs, value, absy_w_data_wait);
}

// Accumulator addressing (implied)
static inline void addr_acc(void) {
    // Dummy read from PC
    WAIT_READY_THEN_READ(cpu.pc, acc_dummy_wait);
    cpu.fetched = cpu.a;
}
...
// Implied addressing
static inline void addr_imp(void) {
    // Dummy read from PC
    WAIT_READY_THEN_READ(cpu.pc, imp_dummy_wait);
}
...
static inline void addr_zpx_write(uint8_t value) {
    ...
    cpu.addr_abs = (base + cpu.x) & 0xFF;
    WAIT_READY_THEN_WRITE(cpu.addr_abs, value, zpx_w_data_wait);
}
...
static inline void addr_zpy_write(uint8_t value) {
    ...
    cpu.addr_abs = (base + cpu.y) & 0xFF;
    WAIT_READY_THEN_WRITE(cpu.addr_abs, value, zpy_w_data_wait);
}
...
static inline void addr_zp_ind_y_write(uint8_t value) {
    ...
    // Always dummy read for writes
    WAIT_READY_THEN_READ(base, zp_ind_y_w_dummy_wait);
    WAIT_READY_THEN_WRITE(cpu.addr_abs, value, zp_ind_y_w_data_wait);
}
...
static inline void addr_rmw_zp(uint8_t (*operation)(uint8_t)) {
    ...
    WAIT_READY_THEN_WRITE(cpu.addr_abs, cpu.fetched, rmw_zp_write_wait);
}
...
static inline void addr_rmw_zpx(uint8_t (*operation)(uint8_t)) {
    ...
    cpu.fetched = operation(cpu.fetched);
    WAIT_READY_THEN_WRITE(cpu.addr_abs, cpu.fetched, rmw_zpx_write_wait);
}
...
static inline void addr_rmw_abs(uint8_t (*operation)(uint8_t)) {
    ...
    cpu.fetched = operation(cpu.fetched);
    WAIT_READY_THEN_WRITE(cpu.addr_abs, cpu.fetched, rmw_abs_write_wait);
}
...
static inline void addr_rmw_absx(uint8_t (*operation)(uint8_t)) {
    ...
    cpu.fetched = operation(cpu.fetched);
    WAIT_READY_THEN_WRITE(cpu.addr_abs, cpu.fetched, rmw_absx_write_wait);
}