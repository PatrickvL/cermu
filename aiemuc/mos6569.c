...
void mos6569_cycle(mos6569_t* vicii) {
    ...
    if (vicii->badline_condition && vicii->raster_cycle >= 15 && vicii->raster_cycle <= 54) {
        vicii->bus->control_lines &= ~BA_LINE; // Pull BA low - CPU will stall
    } else {
        vicii->bus->control_lines |= BA_LINE;  // Release BA
    }
    
    // AEC follows BA with one cycle delay (hardware accurate)
    if (vicii->prev_ba && !(vicii->bus->control_lines & BA_LINE)) {
        vicii->bus->control_lines &= ~AEC_LINE;
    } else if (!vicii->prev_ba && (vicii->bus->control_lines & BA_LINE)) {
        vicii->bus->control_lines |= AEC_LINE;
    }
    vicii->prev_ba = (vicii->bus->control_lines & BA_LINE) != 0;
}
