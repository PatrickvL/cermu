...
void inc_zero_page_func(cpu6510_state_t* cpu_dev) {
    ...
    CPU_INTRA_CYCLE(cpu_dev);
    cpu_write_cycle(cpu_dev, cpu_dev->address, value); // Dummy write
    value++;
...
void inc_absolute_x_func(cpu6510_state_t* cpu_dev) {
    ...
    CPU_INTRA_CYCLE(cpu_dev);
    cpu_write_cycle(cpu_dev, cpu_dev->address + cpu_dev->x, value); // Dummy write
    value++;
    cpu_set_zn(cpu_dev, value);
    CPU_INTRA_CYCLE(cpu_dev);
    cpu_write_cycle(cpu_dev, cpu_dev->address + cpu_dev->x, value);
    CPU_OPCODE_FOOTER(cpu_dev);
}
