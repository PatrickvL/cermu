...
void lax_zero_page_x_func(void) {
    ...
    cpu.addr_abs = (bus.data + cpu.x) & 0xFF;
...
void xaa_immediate_func(void) {
    ...
    cpu.a = (cpu.a | 0xEE) & cpu.x & bus.data; // Unstable, but this is a common guess
    cpu_set_zn(cpu.a);
...
void las_absolute_y_func(void) {
    ...
    cpu.a = cpu.x = cpu.sp = bus.data & cpu.sp;
    cpu_set_zn(cpu.a);
