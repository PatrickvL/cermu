...
These systems lack a dedicated video chip in real hardware. Video output is generated by discrete TTL logic, memory-mapped video RAM, or (for virtual machines) directly by the CPU/interpreter. Systems with identifiable video generation circuitry now delegate to standalone video generator types (`src/chip/video/`), while purely software-driven systems (virtual machines) render directly in system code.

Video generators are **stream-only**: they have no `IndexedFrameBuffer` dependency, maintain an internal pixel buffer, and drive only `CompositeVideoStream`. Systems call `video_port_->bind_display()` for automatic stream→framebuffer reconstruction, following the same pattern as chip-wired systems.

| System | Resolution | Hardware Video Source | Video Generator Type |
|---|---|---|---|
| CHIP-8 | 128×64 | Virtual machine — software interpreter, no physical hardware | N/A (system-level) |
| KC85/2, /3 | 320×256 | Memory-mapped IRM + discrete TTL video logic | `KC85VideoGenerator` (Standard mode) |
| KC85/4 | 320×256 | U82720 ASIC — dual-plane column-major with per-byte color | `KC85VideoGenerator` (Extended mode) |
| Z9001 / KC 87 | 320×192 | Character ROM + TTL logic, optional color RAM (KC 87) | `CharDisplayGenerator` |
| Z1013 | 256×256 | Character ROM + minimal TTL logic (monochrome) | `CharDisplayGenerator` |
| Bomb Jack | 256×224 | Custom tile/sprite TTL logic (3bpp planar, flip flags) | `BombJackVideo` |
| Namco (Pac-Man) | 224×288 | TTL video counters + tile/sprite logic (90° rotation) | `NamcoVideo` |
