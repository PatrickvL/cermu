...
constexpr std::array<opcode_info_t, 256> generate_opcode_table_for_traits(const fam65xx::CPUTraits& traits) {
    ...
    if (traits.has(fam65xx::CMOS_BASE)) {
        ...
        // Based on test failures, many need to be 2-byte NOPs (AM_IMM) to match expected PC advancement
        ...
        table[0x1F] = {OP_NOP, AM_ABS, OF_NONE};  // SLO -> 3-byte NOP (absolute,X)
        ...
        table[0x5F] = {OP_NOP, AM_ABS, OF_NONE};  // SRE -> 3-byte NOP (absolute,X)
