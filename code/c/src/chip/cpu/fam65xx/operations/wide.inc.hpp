        ...
        switch (this->cycle_index) {
            ...
            case 2:
                ...
                if (this->should_complete_write_cycle(pins)) {
                    pins = this->phi2_write(pins, this->get(REG_SP), this->get(REG_ABH));
                    this->dec(REG_S);
                    cycle_index++;
                }
                return pins;
            ...
            case 3:
                ...
                if (should_complete_write_cycle(pins)) {
                    pins = phi2_write(pins, this->get(REG_SP), this->get(REG_ABL)); // addr_low from case 0
                    this->dec(REG_S);
                    transition_to_fetch();
                }
                return pins;
...
bus_state_t op_phb(bus_state_t pins) {
    if constexpr (has_wide_registers()) {
        // Push DBR to stack
        if (should_complete_write_cycle(pins)) {
            pins = phi2_write(pins, this->get(REG_SP), this->wide_state.DBR);
            this->dec(REG_S);
            transition_to_fetch();
        }
    }
        ...
        switch (this->cycle_index) {
            case 0:
                ...
                if (this->should_complete_write_cycle(pins)) {
                    ...
                    pins = this->phi2_write(pins, this->get(REG_SP), d_high);
                    this->dec(REG_S);
                    this->cycle_index++;
                }
                return pins;
            ...
            case 1:
                ...
                if (this->should_complete_write_cycle(pins)) {
                    ...
                    pins = this->phi2_write(pins, this->get(REG_SP), d_low);
                    this->dec(REG_S);
                    this->transition_to_fetch();
                }
                return pins;
...
bus_state_t op_phk(bus_state_t pins) {
    if constexpr (has_wide_registers()) {
        // Push PBR to stack
        if (should_complete_write_cycle(pins)) {
            pins = phi2_write(pins, this->get(REG_SP), this->wide_state.PBR);
            this->dec(REG_S);
            transition_to_fetch();
        }
    }
    ...
    if constexpr (has_wide_registers()) {
        switch (cycle_index) {
            ...
            case 4:
                ...
                if (should_complete_write_cycle(pins)) {
                    pins = phi2_write(pins, this->get(REG_SP), this->get(REG_PCH));
                    this->dec(REG_S);
                    cycle_index++;
                }
                return pins;
            ...
            case 5:
                // Push PC low byte
                if (should_complete_write_cycle(pins)) {
                    pins = phi2_write(pins, this->get(REG_SP), this->get(REG_PCL));
                    this->dec(REG_S);
