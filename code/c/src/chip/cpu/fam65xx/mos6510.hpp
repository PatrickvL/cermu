...
public:
    ...
    bus_state_t init(const fam65xx_desc_t* desc = nullptr) {
        // Opcode table is automatically initialized by mos6510_cpu_t constructor
    ...
    bus_state_t tick(bus_state_t pins) { return cpu_wrapper.tick(pins); }
    ...
    // Direct CPU access
    MOS6510* get_cpu() { return cpu_wrapper.get_cpu(); }
    const MOS6510* get_cpu() const { return cpu_wrapper.get_cpu(); }
};

// Convenient creation functions
// Convenient creation function for basic MOS6510
inline fam65xx_t create() {
    mos6510_cpu_t cpu_wrapper; // Automatically initializes opcode table
...
static inline uint64_t mos6510_init(mos6510_c_t* cpu, const fam65xx_desc_t* desc) {
    cpu->io_ddr = 0x00;
    ...
    return result;
#else
    // C-only fallback: use basic fam65xx_init
    uint64_t result = fam65xx_init(&cpu->impl, desc);
    // Note: C code needs to call opcode table initialization separately
    return result;
#endif
}

// Reset MOS 6510 CPU
static inline uint64_t mos6510_reset(mos6510_c_t* cpu, uint64_t pins) {
    cpu->io_ddr = 0x00;
    cpu->io_data = 0x37;
    cpu->io_external = 0xFF;
...
// Execute one tick
static inline uint64_t mos6510_tick(mos6510_c_t* cpu, uint64_t pins) {
    return fam65xx_tick(&cpu->impl, pins);
}

// Check if operation is done
static inline bool mos6510_opdone(mos6510_c_t* cpu) {
    return fam65xx_opdone(&cpu->impl);
}

// Register access functions
static inline uint8_t mos6510_a(mos6510_c_t* cpu) { return fam65xx_a(&cpu->impl); }
static inline uint8_t mos6510_x(mos6510_c_t* cpu) { return fam65xx_x(&cpu->impl); }
static inline uint8_t mos6510_y(mos6510_c_t* cpu) { return fam65xx_y(&cpu->impl); }
static inline uint8_t mos6510_s(mos6510_c_t* cpu) { return fam65xx_s(&cpu->impl); }
static inline uint8_t mos6510_p(mos6510_c_t* cpu) { return fam65xx_p(&cpu->impl); }
static inline uint16_t mos6510_pc(mos6510_c_t* cpu) { return fam65xx_pc(&cpu->impl); }

static inline void mos6510_set_a(mos6510_c_t* cpu, uint8_t v) { fam65xx_set_a(&cpu->impl, v); }
static inline void mos6510_set_x(mos6510_c_t* cpu, uint8_t v) { fam65xx_set_x(&cpu->impl, v); }
static inline void mos6510_set_y(mos6510_c_t* cpu, uint8_t v) { fam65xx_set_y(&cpu->impl, v); }
static inline void mos6510_set_s(mos6510_c_t* cpu, uint8_t v) { fam65xx_set_s(&cpu->impl, v); }
static inline void mos6510_set_p(mos6510_c_t* cpu, uint8_t v) { fam65xx_set_p(&cpu->impl, v); }
static inline void mos6510_set_pc(mos6510_c_t* cpu, uint16_t v) { fam65xx_set_pc(&cpu->impl, v); }

// I/O Port functions
static inline void mos6510_set_io_ddr(mos6510_c_t* cpu, uint8_t v) { cpu->io_ddr = v; }
static inline void mos6510_set_io_data(mos6510_c_t* cpu, uint8_t v) { cpu->io_data = v; }
static inline void mos6510_set_io_external(mos6510_c_t* cpu, uint8_t v) { cpu->io_external = v; }

static inline uint8_t mos6510_get_io_ddr(mos6510_c_t* cpu) { return cpu->io_ddr; }
static inline uint8_t mos6510_get_io_data(mos6510_c_t* cpu) {
    return (cpu->io_data & cpu->io_ddr) | (cpu->io_external & ~cpu->io_ddr);
}
static inline uint8_t mos6510_get_io_external(mos6510_c_t* cpu) { return cpu->io_external; }
...
static inline mos6510_chip_t* mos6510_chip_create_impl(void) {
...
static inline void mos6510_destroy(mos6510_chip_t* cpu) {
    if (cpu) {
        free(cpu);
    }
}
...
static inline void mos6510_chip_init(mos6510_chip_t* cpu,
                       void (*io_callback)(uint16_t addr, uint8_t data, bool write)) {
    if (cpu) {
...
static inline bus_state_t mos6510_chip_tick(mos6510_chip_t* cpu, bus_state_t bus_state) {
    if (!cpu) return 0;
    return fam65xx_tick(&cpu->cpu_impl, bus_state);
}

// Debug interface
static inline uint16_t mos6510_get_pc(mos6510_chip_t* cpu) {
    return cpu ? fam65xx_pc(&cpu->cpu_impl) : 0;
}

static inline uint8_t mos6510_get_a(mos6510_chip_t* cpu) {
    return cpu ? fam65xx_a(&cpu->cpu_impl) : 0;
}

static inline uint8_t mos6510_get_x(mos6510_chip_t* cpu) {
    return cpu ? fam65xx_x(&cpu->cpu_impl) : 0;
}

static inline uint8_t mos6510_get_y(mos6510_chip_t* cpu) {
    return cpu ? fam65xx_y(&cpu->cpu_impl) : 0;
}

static inline uint8_t mos6510_get_s(mos6510_chip_t* cpu) {
    return cpu ? fam65xx_s(&cpu->cpu_impl) : 0;
}

static inline uint8_t mos6510_get_p(mos6510_chip_t* cpu) {
    return cpu ? fam65xx_p(&cpu->cpu_impl) : 0;
}

// System integration functions
static inline bus_state_t mos6510_tick_chip(void* chip, bus_state_t bus_state) {
