...
void nop_func(mos6510_t* cpu_dev) {
...
void nop_absolute_func(mos6510_t* cpu_dev) {
    CPU_INTRA_CYCLE(cpu_dev);
    ...
    CPU_INTRA_CYCLE(cpu_dev);
    (void)mos6510_read_cycle(cpu_dev, cpu_dev->address);  // Read and discard
    CPU_OPCODE_FOOTER(cpu_dev);
}

void nop_absolute_x_func(mos6510_t* cpu_dev) {
    CPU_INTRA_CYCLE(cpu_dev);
    uint8_t addr_lo = mos6510_read_cycle(cpu_dev, cpu_dev->pc++);
    CPU_INTRA_CYCLE(cpu_dev);
    uint8_t addr_hi = mos6510_read_cycle(cpu_dev, cpu_dev->pc++);
    ...
    if ((base & 0xFF00) != (cpu_dev->address & 0xFF00)) {
        CPU_INTRA_CYCLE(cpu_dev);
        (void)mos6510_read_cycle(cpu_dev, base); // Dummy read from base
    }

    CPU_INTRA_CYCLE(cpu_dev);
    (void)mos6510_read_cycle(cpu_dev, cpu_dev->address);
    CPU_OPCODE_FOOTER(cpu_dev);
}

void nop_immediate_func(mos6510_t* cpu_dev) {
    CPU_INTRA_CYCLE(cpu_dev);
    (void)mos6510_read_cycle(cpu_dev, cpu_dev->pc++);  // Read and discard immediate value
    CPU_OPCODE_FOOTER(cpu_dev);
}

void nop_zero_page_func(mos6510_t* cpu_dev) {
    CPU_INTRA_CYCLE(cpu_dev);
...
void nop_zero_page_x_func(mos6510_t* cpu_dev) {
    CPU_INTRA_CYCLE(cpu_dev);
    cpu_dev->address = mos6510_read_cycle(cpu_dev, cpu_dev->pc++);
    CPU_INTRA_CYCLE(cpu_dev);
