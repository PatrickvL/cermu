...
void mos6502_adc_decimal(mos6502_t* cpu, uint8_t operand) {
    ...
    mos6502_family_set_flag(base, FLAG_Z, binary_result_8 == 0);
    mos6502_family_set_flag(base, FLAG_N, (binary_result_8 & 0x80) != 0);
...
void mos6502_adc_binary(mos6502_t* cpu, uint8_t operand) {
    ...
    mos6502_family_set_flag(base, FLAG_Z, result_8 == 0);
    mos6502_family_set_flag(base, FLAG_N, (result_8 & 0x80) != 0);
...
void mos6502_sbc_decimal(mos6502_t* cpu, uint8_t operand) {
    ...
    mos6502_family_t* base = &cpu->base;
    bool borrow = !mos6502_family_get_flag(base, FLAG_C); // Carry clear means borrow
    ...
    mos6502_family_set_flag(base, FLAG_C, !borrow_out); // Carry set means no borrow
    mos6502_family_set_flag(base, FLAG_Z, binary_result_8 == 0);
    mos6502_family_set_flag(base, FLAG_N, (binary_result_8 & 0x80) != 0);
...
void mos6502_sbc_binary(mos6502_t* cpu, uint8_t operand) {
    ...
    mos6502_family_t* base = &cpu->base;
    bool borrow = !mos6502_family_get_flag(base, FLAG_C);
    
    uint16_t result = base->a - operand - (borrow ? 1 : 0);
    uint8_t result_8 = result & 0xFF;
    
    mos6502_family_set_flag(base, FLAG_C, (result & 0x8000) == 0); // No borrow
    mos6502_family_set_flag(base, FLAG_Z, result_8 == 0);
    mos6502_family_set_flag(base, FLAG_N, (result_8 & 0x80) != 0);
...
void mos6502_op_adc_imm(mos6502_t* cpu) {
    uint8_t operand = mos6502_family_addr_imm(&cpu->base);
    if (mos6502_family_get_flag(&cpu->base, FLAG_D)) {
        mos6502_adc_decimal(cpu, operand);
    } else {
        mos6502_adc_binary(cpu, operand);
    }
    MOS6502_FAMILY_OPCODE_FOOTER(&cpu->base);
}

void mos6502_op_adc_zp(mos6502_t* cpu) {
    uint8_t operand = mos6502_family_addr_zp(&cpu->base);
    if (mos6502_family_get_flag(&cpu->base, FLAG_D)) {
        mos6502_adc_decimal(cpu, operand);
    } else {
        mos6502_adc_binary(cpu, operand);
...
void mos6502_op_adc_zpx(mos6502_t* cpu) {
    uint8_t operand = mos6502_family_addr_zpx(&cpu->base);
    if (mos6502_family_get_flag(&cpu->base, FLAG_D)) {
        mos6502_adc_decimal(cpu, operand);
    } else {
        mos6502_adc_binary(cpu, operand);
...
void mos6502_op_adc_abs(mos6502_t* cpu) {
    uint8_t operand = mos6502_family_addr_abs(&cpu->base);
    if (mos6502_family_get_flag(&cpu->base, FLAG_D)) {
        mos6502_adc_decimal(cpu, operand);
    } else {
        mos6502_adc_binary(cpu, operand);
...
void mos6502_op_adc_absx(mos6502_t* cpu) {
    uint8_t operand = mos6502_family_addr_absx(&cpu->base);
    if (mos6502_family_get_flag(&cpu->base, FLAG_D)) {
        mos6502_adc_decimal(cpu, operand);
    } else {
        mos6502_adc_binary(cpu, operand);
...
void mos6502_op_adc_absy(mos6502_t* cpu) {
    uint8_t operand = mos6502_family_addr_absy(&cpu->base);
    if (mos6502_family_get_flag(&cpu->base, FLAG_D)) {
        mos6502_adc_decimal(cpu, operand);
    } else {
        mos6502_adc_binary(cpu, operand);
...
void mos6502_op_adc_indx(mos6502_t* cpu) {
    uint8_t operand = mos6502_family_addr_indx(&cpu->base);
    if (mos6502_family_get_flag(&cpu->base, FLAG_D)) {
        mos6502_adc_decimal(cpu, operand);
    } else {
        mos6502_adc_binary(cpu, operand);
...
void mos6502_op_adc_indy(mos6502_t* cpu) {
    uint8_t operand = mos6502_family_addr_indy(&cpu->base);
    if (mos6502_family_get_flag(&cpu->base, FLAG_D)) {
        mos6502_adc_decimal(cpu, operand);
    } else {
        mos6502_adc_binary(cpu, operand);
...
void mos6502_op_brk(mos6502_t* cpu) {
    ...
    // Jump to IRQ vector
    MOS6502_FAMILY_INTRA_CYCLE(base);
