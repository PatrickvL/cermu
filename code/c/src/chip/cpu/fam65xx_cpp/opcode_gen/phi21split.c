...
static Pins cpu_phi2_access(CPU6502* cpu, Pins pins, uint16_t address, uint8_t reg_write);
 ...
 *   reg_write: Register index to write from (REG_DUMMY for read cycles)
 */
static Pins cpu_phi2_access(CPU6502* cpu, Pins pins, uint16_t address, uint8_t reg_write) {
    bool is_write = (reg_write != REG_DUMMY);
...
static Pins addr_zp(CPU6502* cpu, Pins pins) {
    ...
    pins = cpu_phi2_access(cpu, pins, CPU_PC(cpu), REG_DUMMY);
...
static Pins addr_zpx(CPU6502* cpu, Pins pins) {
    switch (cpu->cycle_index++) {
        ...
        case 1:
            ...
            pins = cpu_phi2_access(cpu, pins, CPU_ZP(cpu), REG_DUMMY);
            if (CPU_GET_HALT(pins)) return pins;
...
static Pins addr_abx(CPU6502* cpu, Pins pins) {
    switch (cpu->cycle_index++) {
        ...
        case 2:
            ...
            pins = cpu_phi2_access(cpu, pins, CPU_AB(cpu), REG_DUMMY);
            if (CPU_GET_HALT(pins)) return pins;
...
static Pins op_sta(CPU6502* cpu, Pins pins) {
    ...
    pins = cpu_phi2_access(cpu, pins, CPU_AB(cpu), REG_A);
    if (CPU_GET_HALT(pins)) return pins;
...
static Pins op_stx(CPU6502* cpu, Pins pins) {
    ...
    pins = cpu_phi2_access(cpu, pins, CPU_AB(cpu), REG_X);
    if (CPU_GET_HALT(pins)) return pins;
...
static Pins op_sty(CPU6502* cpu, Pins pins) {
    ...
    pins = cpu_phi2_access(cpu, pins, CPU_AB(cpu), REG_Y);
    if (CPU_GET_HALT(pins)) return pins;
    ...
    if (cpu->opcode_entry.rmw) {
        ...
        switch (cpu->cycle_index++) {
            ...
            case 1:
                ...
                pins = cpu_phi2_access(cpu, pins, CPU_AB(cpu), REG_DL);
                if (CPU_GET_HALT(pins)) return pins;
