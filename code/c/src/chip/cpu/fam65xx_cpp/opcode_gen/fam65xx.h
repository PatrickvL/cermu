...
// API functions - updated to use bus_state_t for proper type safety
...
#define STACK_PEEK() do { \
    BUS_SET_ADDR(pins, c->SP); /* Use current SP for address without changing it */ \
    c->reg_idx = R_DL; \
} while(0)
...
static inline bus_state_t _fam65xx_decode(fam65xx_t* c, bus_state_t pins);
...
bus_state_t fam65xx_init(fam65xx_t* c, const fam65xx_desc_t* desc) {
    CHIPS_ASSERT(c && desc);
    ...
    bus_state_t pins = 0;
    pins |= BUS_BIT(BUS_RDY_BIT);  // Set ready bit using system_lines.h definitions
    
    return pins;
}

// Bootstrap function for proper CPU initialization
bus_state_t fam65xx_bootstrap(fam65xx_t* c, bus_state_t pins) {
    CHIPS_ASSERT(c);
    
    // Only bootstrap if CPU is in uninitialized state
    if (c->CI == 0xFFFF) {
        // Set up for first instruction fetch
        pins |= BUS_BIT(BUS_RDY_BIT);   // Ensure RDY is high for execution
        pins |= BUS_BIT(BUS_RW_BIT);    // Ensure RW is set for read operation
...
bus_state_t fam65xx_tick(fam65xx_t* c, bus_state_t pins) {
    CHIPS_ASSERT(c);
    
    // RDY check: stall CPU BEFORE calling decode if not ready
    if (!(pins & BUS_BIT(BUS_RDY_BIT))) {
    ...
    if (pins & BUS_BIT(BUS_SYNC_BIT)) {
        pins &= ~BUS_BIT(BUS_SYNC_BIT);
        
        // Shift interrupt pipelines (required for hardware accuracy)
        c->nmi_pip = ((c->nmi_pip << 1) | ((pins & BUS_BIT(BUS_NMI_BIT)) ? 0x01 : 0x00)) & 0xFF;
        c->irq_pip = ((c->irq_pip << 1) | ((pins & BUS_BIT(BUS_IRQ_BIT)) ? 0x01 : 0x00)) & 0xFF;
...
bus_state_t fam6510_iorq(fam65xx_t* c, bus_state_t pins) {
