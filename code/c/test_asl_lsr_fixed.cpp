// Fixed test for ASL A and LSR A with correct cycle count
#include <iostream>
#include <iomanip>
#include <vector>
...
bool test_asl_a() {
    ...
    std::vector<uint8_t> memory(65536, 0xEA); // Fill with NOP to avoid issues
    memory[0x0000] = 0x0A; // ASL A instruction
    
    // ASL A is a 2-cycle instruction: 
    // Cycle 0: Opcode fetch (0x0A) - handled by CPU internally
    // Cycle 1: Execute ASL_ACC operation with SYNC
    ...
    for (int cycle = 0; cycle < 2; cycle++) {
        ...
        std::cout << "Cycle " << cycle << ": PC=0x" << std::hex << cpu.get_pc() 
...
int main() {
    std::cout << "=== Testing Accumulator Operations (Fixed Cycle Count) ===" << std::endl;
