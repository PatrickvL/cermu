...
int main() {
    std::cout << "=== Debug Memory Shift Execution ===" << std::endl;
    
    TestCPU cpu;
    cpu.init_for_test();
    cpu.set_pc(0x1000);
    
    // Test ASL $10 (zero page)
    std::array<uint8_t, 65536> memory = {};
    memory[0x1000] = 0x06;  // ASL $nn opcode
    memory[0x1001] = 0x10;  // Zero page address
    memory[0x0010] = 0x42;  // Value to shift
    
    std::cout << "Testing ASL $10 with detailed cycle analysis" << std::endl;
    std::cout << "Initial Memory[0x10] = 0x" << std::hex << (int)memory[0x0010] << std::endl;
    ...
    for (int cycle = 1; cycle <= 10; cycle++) {
        ...
        std::cout << "\n--- Cycle " << std::dec << cycle << " (BEFORE execution) ---" << std::endl;
        std::cout << "  Address: 0x" << std::hex << std::setw(4) << std::setfill('0') << addr;
        std::cout << ", R/W: " << (is_read ? "READ" : "WRITE");
        std::cout << ", Step: " << std::dec << (int)step_before;
        std::cout << ", PC: 0x" << std::hex << std::setw(4) << pc_before << std::endl;
        
        // Check cycle table for this step
        if (step_before > 0) {
            auto cycle_desc = TestCPU::GET_CYCLE(0x06, step_before);
        ...
        std::cout << "--- Cycle " << std::dec << cycle << " (AFTER execution) ---" << std::endl;
        std::cout << "  Step: " << std::dec << (int)step_after;
        std::cout << ", PC: 0x" << std::hex << std::setw(4) << pc_after;
        std::cout << ", Memory[0x10]: 0x" << std::setw(2) << (int)memory[0x0010];
    ...
    std::cout << "Memory[0x10] = 0x" << std::hex << std::setw(2) << (int)memory[0x0010] << " (expected 0x84)" << std::endl;
