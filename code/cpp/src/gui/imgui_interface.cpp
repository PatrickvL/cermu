...
void gui_init_state(gui_state_t *gui_state) {
  ...
  gui_state->show_chip_visualization_config =
      false; // Global chip visualization config dialog
...
void gui_render_frame(c64_t *c64, gui_state_t *gui_state,
  ...
  if (gui_state->show_chip_visualization_config) {
    GlobalChipStyleManager::getInstance().renderConfigDialog(
        &gui_state->show_chip_visualization_config);
  }
  ...
  if (c64 && c64->system.chip_count > 0) {
    for (uint8_t chip_id = 0; chip_id < c64->system.chip_count && chip_id < 16;
         chip_id++) {
      chip_entry_t *entry = &c64->system.chips[chip_id];

      // Render debug window if enabled and chip has debug callback
      if (gui_state->show_chip_debug[chip_id] && entry->desc &&
          entry->desc->render_debug_window) {
        entry->desc->render_debug_window(entry->chip,
                                         &gui_state->show_chip_debug[chip_id]);
      }

      // Render settings window if enabled and chip has settings callback
      if (gui_state->show_chip_settings[chip_id] && entry->desc &&
          entry->desc->render_settings_window) {
        entry->desc->render_settings_window(
            entry->chip, &gui_state->show_chip_settings[chip_id]);
      }
    }
  }
...
void gui_render_menu_bar(c64_t *c64, gui_state_t *gui_state,
  ...
  if (ImGui::BeginMainMenuBar()) {
    ...
    if (ImGui::BeginMenu("Emulation")) {
      ...
      ImGui::Separator();
      // Start/Pause button
      bool is_running = emu_context
                            ? (emu_context->current_state == EMU_STATE_RUNNING)
                            : gui_state->emulation_running;
      const char *run_pause_text = is_running ? "Pause" : "Start";
      if (ImGui::MenuItem(run_pause_text, nullptr, false,
                          emu_context != nullptr)) {
        if (emu_context) {
          if (is_running) {
      ...
      // Single step button
      if (ImGui::MenuItem("Single Step", nullptr, false,
                          emu_context != nullptr &&
                              !gui_state->emulation_running)) {
        if (emu_context) {
      ...
      if (emu_context) {
        ...
        if (ImGui::SliderFloat("Speed", &speed_multiplier, 0.1f, 5.0f,
                               "%.1fx")) {
    ...
    if (ImGui::BeginMenu("View")) {
      ...
      ImGui::Separator();
      ImGui::MenuItem("Chip Visualization Config", nullptr,
                      &gui_state->show_chip_visualization_config);
    ...
    if (ImGui::BeginMenu("Screen")) {
      ...
      ImGui::SliderFloat("Scale", &gui_state->screen_scale, 0.5f, 4.0f, "%.1fx",
      ...
      if (gui_state->aspect_ratio_mode == ASPECT_RATIO_CUSTOM) {
        ImGui::SliderFloat("Custom Ratio", &gui_state->custom_aspect_ratio,
                           0.5f, 3.0f, "%.2f", ImGuiSliderFlags_None);
      }

      // Scaling mode selection
      const char *scaling_mode_items[] = {"Fit (Black Bars)", "Fill (Crop)",
                                          "Stretch", "Integer Scale"};
      int current_scaling = (int)gui_state->scaling_mode;
      if (ImGui::Combo("Scaling Mode", &current_scaling, scaling_mode_items,
                       SCALING_MODE_COUNT, -1)) {
        gui_state->scaling_mode = (scaling_mode_t)current_scaling;
      }

      // Additional options
      ImGui::Checkbox("Maintain Pixel Aspect",
                      &gui_state->maintain_pixel_aspect);
      ...
      if (color_cycle_enabled && c64 && c64->vicii) {
        ...
        if (current_time - last_cycle_time > 2000) { // Change every 2 seconds
          // Write to VIC-II background color register - this affects the center
          // area
          ...
          bus_state_t border_bus_state =
              BUS_STATE(VICII_EC, (current_bg_color + 8) % 16, 0);
          vicii_registers_write(c64->vicii, border_bus_state);

          current_bg_color = (current_bg_color + 1) % 16;
          last_cycle_time = current_time;
        }
        ImGui::Text("Current BG Color: %d",
                    (current_bg_color + 15) % 16); // Show the current one
      }
      ...
      // Display information - use the constants that are defined later in the
      // file
      ...
      if (c64 && c64->system.chip_count > 0) {
        if (ImGui::BeginMenu("Debug Windows")) {
          for (uint8_t chip_id = 0;
               chip_id < c64->system.chip_count && chip_id < 16; chip_id++) {
            chip_entry_t *entry = &c64->system.chips[chip_id];
            if (entry->desc && entry->desc->render_debug_window) {
              ...
              ImGui::MenuItem(menu_label, NULL,
                              &gui_state->show_chip_debug[chip_id]);
          ...
          for (uint8_t chip_id = 0;
            ...
            if (entry->desc && entry->desc->render_settings_window) {
              ...
              snprintf(menu_label, sizeof(menu_label), "%s Settings",
                       desc ? desc : "Unknown Chip");
              ImGui::MenuItem(menu_label, NULL,
                              &gui_state->show_chip_settings[chip_id]);
              ImGui::PopID(); // Pop chip ID
            }
          }
          ImGui::EndMenu();
      ...
      const char *state_text =
          emu_context->current_state == EMU_STATE_RUNNING
              ...
              : emu_context->current_state == EMU_STATE_PAUSED
                    ? "Paused"
                    : emu_context->current_state == EMU_STATE_STEPPING
                          ? "Stepping"
                          : emu_context->current_state == EMU_STATE_STOPPED
                                ? "Stopped"
                                : "Unknown";
...
void gui_render_memory_viewer(c64_t *c64, gui_state_t *gui_state) {
  ...
  // Address input
  ImGui::InputInt("Address", (int *)&gui_state->memory_address, 1, 16,
                  ImGuiInputTextFlags_CharsHexadecimal);
  gui_state->memory_address &= 0xFFFF; // Keep within 16-bit range

  ImGui::SliderInt("Columns", &gui_state->memory_columns, 8, 32, "%d",
                   ImGuiSliderFlags_None);
  ...
  if (c64) {
    ...
    for (int row = 0; row < 16; row++) {
      uint16_t addr =
          gui_state->memory_address + (row * gui_state->memory_columns);
      ImGui::Text("%04X: ", addr);

      for (int col = 0; col < gui_state->memory_columns; col++) {
        uint16_t byte_addr = addr + col;
        if (col >= gui_state->memory_columns || addr + col > 0xFFFF)
          break;
...
void gui_render_debugger(c64_t *c64, gui_state_t *gui_state,
  ...
  if (emu_context) {
    ...
    if (c64) {
      ...
      // Check reset vector
      if (c64) {
        // Read reset vector through proper memory mapping (ROM or RAM depending
        // on banking)
  ...
  ImGui::InputInt("Address", (int *)&gui_state->breakpoint_address, 1, 16,
                  ImGuiInputTextFlags_CharsHexadecimal);
...
void gui_render_settings(c64_t *c64, gui_state_t *gui_state) {
  ...
  ImGui::SliderInt("Target FPS", &gui_state->target_fps, 25, 120, "%d",
                   ImGuiSliderFlags_None);
  ImGui::SliderFloat("Emulation Speed", &gui_state->emulation_speed, 0.1f,
                     10.0f, "%.1fx", ImGuiSliderFlags_None);

  ImGui::Separator();
  ImGui::Text("ROM Files");

  ImGui::InputText("BASIC ROM", gui_state->rom_path_basic,
...
#define VIC_OFFSET_X                                                           \
  ((C64_TOTAL_WIDTH - C64_VISIBLE_WIDTH) / 2) // Center horizontally
#define VIC_OFFSET_Y                                                           \
  ((C64_TOTAL_HEIGHT - C64_VISIBLE_HEIGHT) / 2) // Center vertically
...
// Remove vicii_buffer - VIC-II will render directly to centered area of
// screen_buffer

// Get access to the screen buffer for VIC-II framebuffer setup
uint32_t *gui_get_screen_buffer(void) { return screen_buffer; }

// Get screen buffer dimensions
void gui_get_screen_dimensions(int *width, int *height) {
  if (width)
    *width = C64_TOTAL_WIDTH;
  if (height)
    *height = C64_TOTAL_HEIGHT;
}
...
bool gui_init_screen_display(gui_state_t *gui_state) {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, C64_TOTAL_WIDTH, C64_TOTAL_HEIGHT, 0,
...
// Re-add vicii_buffer but as a temporary solution until VIC-II can handle
// stride
...
void gui_update_screen_texture(c64_t *c64, gui_state_t *gui_state) {
  ...
  // When showing invisible area, use brownish color (VIC-II generates color 9 =
  // brown in invisible area)
  ...
  if (c64 && c64->vicii) {
    ...
    if (connected_vicii_chip != c64->vicii) {
      ...
      vicii_set_framebuffer((vicii_t *)c64->vicii, vicii_buffer,
...
void gui_render_screen(c64_t *c64, gui_state_t *gui_state) {
  ...
  if (gui_state->host_dpi_scale <= 0.0f) {
    gui_state->host_dpi_scale = io.DisplayFramebufferScale.x > 0.0f
  ...
  if (ImGui::Begin("##C64ScreenBackground", NULL, window_flags)) {
    ...
    gui_calculate_display_dimensions(
        gui_state, viewport->Size.x, viewport->Size.y, guest_width,
        guest_height, is_pal, &display_width, &display_height, &pos_x, &pos_y);
    ...
    // When show_invisible_area is true, use full texture (uv_min/max = 0,0 to
    // 1,1)
    ...
    if (ImGui::IsItemHovered(ImGuiHoveredFlags_None)) {
      ...
      if (rel_x >= 0.0f && rel_x <= 1.0f && rel_y >= 0.0f && rel_y <= 1.0f &&
          guest_x >= 0 && guest_x < (int)guest_width && guest_y >= 0 &&
          guest_y < (int)guest_height) {

        if (gui_state->show_overscan) {
          ImGui::SetTooltip("C64 Screen coordinates: (%d, %d)", guest_x,
                            guest_y);
        } else {
...
float gui_get_target_aspect_ratio(gui_state_t *gui_state, bool is_pal) {
  ...
  default: {
    ...
    // C64 pixels are not square - they have different aspect ratios for PAL vs
    // NTSC
...
void gui_calculate_display_dimensions(gui_state_t *gui_state,
                                      float viewport_width,
                                      float viewport_height, float guest_width,
                                      float guest_height, bool is_pal,
                                      float *out_display_width,
                                      float *out_display_height,
...
bool gui_apply_rom_changes(gui_emulation_context_t *emu_context,
                           gui_state_t *gui_state) {
  if (!emu_context || !emu_context->c64 || !gui_state) {
    return false;
  }

  // Check if any ROM paths have changed (simplified - just reload
...
void gui_load_rom_file(const char *filepath, const char *type,
  ...
  if (strcmp(type, "basic") == 0) {
    strncpy(gui_state->rom_path_basic, filepath,
            sizeof(gui_state->rom_path_basic) - 1);
    gui_state->rom_path_basic[sizeof(gui_state->rom_path_basic) - 1] = '\0';
  } else if (strcmp(type, "kernal") == 0) {
    strncpy(gui_state->rom_path_kernal, filepath,
            sizeof(gui_state->rom_path_kernal) - 1);
    gui_state->rom_path_kernal[sizeof(gui_state->rom_path_kernal) - 1] = '\0';
  } else if (strcmp(type, "chargen") == 0) {
    strncpy(gui_state->rom_path_chargen, filepath,
            sizeof(gui_state->rom_path_chargen) - 1);
...
bool gui_reload_roms_from_state(c64_t *c64, const gui_state_t *gui_state) {
  ...
  // For now, we'll extract just the filename from the full path stored in GUI
  // state and create a custom ROM configuration
  ...
  if (!basic_name)
    basic_name = strrchr(gui_state->rom_path_basic, '\\');
  if (basic_name) {
    strncpy(basic_filename, basic_name + 1, sizeof(basic_filename) - 1);
  } else {
    strncpy(basic_filename, gui_state->rom_path_basic,
            sizeof(basic_filename) - 1);
  }

  const char *kernal_name = strrchr(gui_state->rom_path_kernal, '/');
  if (!kernal_name)
    kernal_name = strrchr(gui_state->rom_path_kernal, '\\');
  if (kernal_name) {
    strncpy(kernal_filename, kernal_name + 1, sizeof(kernal_filename) - 1);
  } else {
    strncpy(kernal_filename, gui_state->rom_path_kernal,
            sizeof(kernal_filename) - 1);
  }

  const char *chargen_name = strrchr(gui_state->rom_path_chargen, '/');
  if (!chargen_name)
    chargen_name = strrchr(gui_state->rom_path_chargen, '\\');
  if (chargen_name) {
    strncpy(chargen_filename, chargen_name + 1, sizeof(chargen_filename) - 1);
  } else {
    strncpy(chargen_filename, gui_state->rom_path_chargen,
            sizeof(chargen_filename) - 1);
  }

  // Create a custom ROM configuration from GUI state
  rom_config_t custom_rom_config = {
      .basic_rom_filenames = {basic_filename, NULL, NULL, NULL, NULL},
      .kernal_rom_filenames = {kernal_filename, NULL, NULL, NULL, NULL},
      .chargen_rom_filenames = {chargen_filename, NULL, NULL, NULL, NULL,
                                NULL}};
...
bool gui_emulation_thread_init(gui_emulation_context_t *context, c64_s *c64) {
  ...
  sdl_thread_impl_t *impl =
      (sdl_thread_impl_t *)calloc(1, sizeof(sdl_thread_impl_t));
  if (!impl)
    return false;
...
void gui_emulation_thread_cleanup(gui_emulation_context_t *context) {
  if (!context || !context->thread_impl)
    return;
...
bool gui_emulation_thread_start(gui_emulation_context_t *context) {
  ...
  impl->thread =
      SDL_CreateThread(gui_emulation_thread_main, "C64Emulation", context);
...
void gui_emulation_send_signal(gui_emulation_context_t *context,
                               emulation_signal_t signal) {
  if (!context || !context->thread_impl)
    return;
...
void gui_emulation_set_speed(gui_emulation_context_t *context,
  ...
  SDL_LockMutex(impl->signal_mutex);
  context->cycles_per_second =
      (uint64_t)(PAL_CYCLES_PER_SECOND * speed_multiplier);
...
static int gui_emulation_thread_main(void *data) {
  ...
  while (context->thread_running) {
    // Wait for signal
    SDL_LockMutex(impl->signal_mutex);
    while (context->pending_signal == EMU_SIGNAL_NONE &&
    ...
    case EMU_SIGNAL_START: {
      ...
      if (reset_vector == 0x0000) {
        printf("Emulation thread: WARNING - No ROM loaded, reset vector is "
               "$0000\n");
        printf("Emulation thread: Using simulation mode instead of real CPU "
               "execution\n");
        ...
        const uint32_t sim_log_interval_ms =
            10000; // Log every 10 seconds for simulation
        while (context->current_state == EMU_STATE_RUNNING &&
               context->thread_running && sim_cycles < MAX_SIM_CYCLES) {
          ...
          if ((sim_cycles % 1000) == 0) {
            if (!context->thread_running) {
              printf("Emulation thread: Intercept or quit detected during "
                     "simulation\n");
              break;
            }
            ...
            if (current_sim_time - last_sim_log_time >= sim_log_interval_ms) {
              printf("Emulation thread: Simulation executed %llu cycles "
                     "(total: %llu)\n",
        ...
        if (sim_cycles >= MAX_SIM_CYCLES) {
          printf("Emulation thread: Simulation reached cycle limit (%llu "
                 "cycles)\n",
                 (unsigned long long)sim_cycles);
        }
      } else {
        ...
        while (context->current_state == EMU_STATE_RUNNING &&
          ...
          if (first_iteration ||
              (loop_start_time - last_log_time) >= log_interval_ms) {
          ...
          // Use cycle-based execution - execute one frame worth of cycles at a
          // time PAL C64: ~19,705 cycles per frame (985,248 Hz / 50 fps)
          for (int i = 0;
               i < 19705 && context->current_state == EMU_STATE_RUNNING &&
               context->thread_running;
               i++) {
            c64_system_tick(context->c64);
            context->total_cycles_executed++;
            ...
            if ((i % 1000) == 0 && !context->thread_running) {
              printf("Emulation thread: Quick quit detected during CPU "
                     "execution\n");
              break;
            }
          }
          ...
          if (execution_time <
              20) { // Less than one frame (50fps = 20ms for PAL)
            SDL_Delay(20 -
                      execution_time); // Sleep to maintain ~50fps PAL timing
          }
          ...
          if (context->current_state != EMU_STATE_RUNNING ||
              !context->thread_running) {
            printf("Emulation thread: Pause/stop requested (state=%d, "
                   "thread_running=%d)\n",
          ...
          if (current_time - last_log_time >= log_interval_ms) {
            uint64_t cycles_executed =
                context->total_cycles_executed - last_total_cycles;
          ...
          // Very small delay to allow GUI responsiveness without hurting
          // performance Main timing control is handled by the frame delay above
          SDL_Delay(0);
        }
      }
    ...
    case EMU_SIGNAL_RESET:
      ...
      printf(
          "Emulation thread: Reset signal received (handled by GUI thread)\n");
      context->current_state = EMU_STATE_STOPPED;
      break;
    ...
    case EMU_SIGNAL_QUIT:
      printf("Emulation thread: Quit signal received, setting thread_running = "
             "false\n");
      context->thread_running = false;
      context->current_state = EMU_STATE_STOPPED;
      break;
...
void gui_delay(uint32_t ms) { SDL_Delay(ms); }
