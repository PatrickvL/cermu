...
bus_state_t op_bra(bus_state_t pins) {
    if constexpr (has_cmos()) {
        ...
        pins = this->phi2_read(pins, static_cast<reg16_t>(REG_PC), static_cast<reg8_t>(REG_DL));
...
bus_state_t op_stz(bus_state_t pins) {
    if constexpr (has_cmos()) {
        ...
        if (this->should_complete_write_cycle(pins)) {
            pins = this->phi2_write(pins, static_cast<reg16_t>(REG_AB), 0x00);
            transition_to_fetch();
        }
    }
    return pins;
    ...
    if constexpr (has_cmos()) {
        ...
        switch (this->cycle_index) {
            case 0:
                ...
                pins = this->phi2_read(pins, static_cast<reg16_t>(REG_AB), static_cast<reg8_t>(REG_DL));
                if (FAM65XX_GET_RDY(pins)) {
                    this->cycle_index++;
                }
                return pins;
            ...
            case 1:
                ...
                if (this->should_complete_write_cycle(pins)) {
                    pins = this->phi2_write(pins, static_cast<reg16_t>(REG_AB), this->get(REG_DL));
    ...
    if constexpr (has_cmos()) {
        switch (this->cycle_index) {
            ...
            case 1:
                ...
                if (this->should_complete_write_cycle(pins)) {
                    pins = this->phi2_write(pins, static_cast<reg16_t>(REG_SP), this->get(REG_X));
                    this->dec(REG_S);
                    transition_to_fetch();
                }
                return pins;
    ...
    if constexpr (has_cmos()) {
        switch (this->cycle_index) {
            ...
            case 1:
                ...
                if (this->should_complete_write_cycle(pins)) {
                    pins = this->phi2_write(pins, static_cast<reg16_t>(REG_SP), this->get(REG_Y));
                    this->dec(REG_S);
                    transition_to_fetch();
                }
                return pins;
    ...
    if constexpr (has_cmos()) {
        switch (this->cycle_index) {
            ...
            case 2:
                ...
                pins = this->phi2_read(pins, static_cast<reg16_t>(REG_SP), static_cast<reg8_t>(REG_X));
                if (FAM65XX_GET_RDY(pins)) {
    ...
    if constexpr (has_cmos()) {
        switch (this->cycle_index) {
            ...
            case 2:
                ...
                pins = this->phi2_read(pins, static_cast<reg16_t>(REG_SP), static_cast<reg8_t>(REG_Y));
                if (FAM65XX_GET_RDY(pins)) {
