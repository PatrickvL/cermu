 ...
 * - Packed bus state representation (pins: data, address, RW, RDY, SYNC,
 * cycle_idx)
 * - Endian-aware register layout with explicit 8/16-bit access
 * - Scheduled bus access pattern (centralized in tick function)
 * - Three address sources: PC (program counter), AB (address bus), SP (stack
 * pointer)
 ...
 * - BA (Bus Available) follows AEC with ~2-3 cycle delay (capacitive delay
 * line)
...
// Operation type constants (for reference - actual code uses Operation enum
// class)
...
// Addressing mode constants (for reference - actual code uses AddressingMode
// enum class)
...
// Opcode flags constants (for reference - actual code uses OpcodeFlags enum
// class)
...
constexpr uint8_t OF_PC_OR_AB =
    0x8; // First operation cycle: PC(1) or AB(0)
...
template <typename CPUTraits> struct BusConfig {
  // Fixed layout:
  ...
  static constexpr uint64_t ADDR_MASK = ((1ULL << ADDR_WIDTH) - 1)
                                        << ADDR_SHIFT;
...
template <typename Config> class BusState {
public:
  // Data access - always zero shift
  static uint8_t get_data(bus_state_t state) { return (uint8_t)state; }
  ...
  static bus_state_t set_addr(bus_state_t state, uint32_t addr) {
    ...
    return (state & ~Config::ADDR_MASK) |
           ((uint64_t)masked << Config::ADDR_SHIFT);
  }
...
template <typename CPUTraits> class CPU {
public:
...
private:
  ...
  inline void do_adc(data_t operand) {
    ...
    if constexpr (data_width == 8) {
      ...
      if (is_8bit_mode()) {
        ...
        set_flag(FLAG_C, result > 0xFF);
        set_flag(FLAG_V, (((a & 0xFF) ^ result) & ((operand & 0xFF) ^ result) &
                          0x80) != 0);
        set_nz_flags((uint8_t)result);
      } else {
  ...
  inline bus_state_t dummy_write(bus_state_t pins, uint32_t addr,
                                 uint8_t data) {
  ...
  bus_state_t transition_to_operation(bus_state_t pins) {
    ...
    AddrReg addr_reg =
        (current_opcode_info.flags & OF_PC_OR_AB) ? AddrReg::PC : AddrReg::AB;
  ...
  bus_state_t addr_absolute_x(bus_state_t pins) {
    ...
    case 2: { // Fix address after page cross
      ...
      uint16_t addr2 =
          (base2 & 0x00FF) + get8(Reg8::X) + ((base2 & 0xFF00) ? 0x0100 : 0);
...
int main() {
  ...
  auto cpu_mem_cb = [](void *ctx, uint16_t addr, uint8_t bus_data,
  ...
  auto dma_mem_cb = [](void *ctx, uint16_t addr, uint8_t bus_data,
                       bool is_write) -> uint8_t {
    // VIC-II DMA access
    uint8_t *ram = (uint8_t *)ctx;
    return ram[addr];
  ...
  MemoryHandlers mem = {.cpu_cb = cpu_mem_cb,
                        .cpu_ctx = ram,
                        .dma_cb = dma_mem_cb,
                        .dma_ctx = ram};
 ...
 * ==============================
 *
 * This reference shows best practices. You can adopt these patterns
 * incrementally:
 *
