...
//   row_bit = VIA Port B bit position (column select on VIC-20 hardware)
//   col_bit = VIA Port A bit position (row read on VIC-20 hardware)
//   Same bit-reversed layout as C64/C16/C128/PET matrices.
//
//   NOTE: On VIC-20 hardware, Port B drives columns and Port A reads rows —
//   the reverse of the C64.  The array's first dimension is still called
//   "row" to match the codebase-wide convention (first dim = row_open_contacts,
//   second dim = col_open_contacts).  The scan callbacks read the contact
//   arrays accordingly.
//
...
static const petscii_t vic20_unshifted_chars[VIC20_KEYBOARD_ROWS * VIC20_KEYBOARD_COLS] = {
    ...
    // row 2: (CRSR→), ;, L, J, G, D, A, (CTRL)
    0, ';', 'L', 'J', 'G', 'D', 'A', 0,
    // row 1: (RETURN), *, P, I, Y, R, W, ←($5F)
    0, '*', 'P', 'I', 'Y', 'R', 'W', 0x5F,
    // row 0: (DEL), £($5C), +, 9, 7, 5, 3, 1
    0, 0x5C, '+', '9', '7', '5', '3', '1',
};
...
static const petscii_t vic20_shifted_chars[VIC20_KEYBOARD_ROWS * VIC20_KEYBOARD_COLS] = {
    // row 7: (F8), (CLR), (-), (0), (, &, $, "
    0, 0, 0, 0, '(', '&', '$', '"',
    // row 6: (F6), π($DE), (@), o($CF), u($D5), t($D4), e($C5), q($D1)
    0, 0xDE, 0, 0xCF, 0xD5, 0xD4, 0xC5, 0xD1,
    // row 5: (F4), (=), [, k($CB), h($C8), f($C6), s($D3), (C=)
    0, 0, '[', 0xCB, 0xC8, 0xC6, 0xD3, 0,
    // row 4: (F2), (RSHIFT), >, m($CD), b($C2), c($C3), z($DA), (SPACE)
    0, 0, '>', 0xCD, 0xC2, 0xC3, 0xDA, 0,
    // row 3: (CRSR↑), ?, <, n($CE), v($D6), x($D8), (LSHIFT), (RUN/STOP)
    0, '?', '<', 0xCE, 0xD6, 0xD8, 0, 0,
    // row 2: (CRSR←), ], l($CC), j($CA), g($C7), d($C4), a($C1), (CTRL)
    0, ']', 0xCC, 0xCA, 0xC7, 0xC4, 0xC1, 0,
    // row 1: (RETURN), (*), p($D0), i($C9), y($D9), r($D2), w($D7), (←)
