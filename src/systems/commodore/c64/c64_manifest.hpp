...
//
// Row arguments:
//   X(ctx, type, chip, base, mask, overlay, label, info_label, rom_files)
//
//   mask — address line mask the chip decodes.
//     MemoryChipBase-derived types: mask+1 = flat-memory buffer size.
//       (e.g. 0x1FFF → 8192 bytes)
//     All other types: MMIO address decode mask, stored as addr_mask.
//       (e.g. 0x3F → 64 register addresses)
//
// All chips become value-typed fields in C64Chipset.  Buffer vs MMIO is
// derived from the chip type via std::is_base_of_v<MemoryChipBase, T>.
// Board::bind_chip() auto-calls on_bind_buffer() for buffer chips
// (MemoryChipBase overrides this).
//
// Chips are sorted by base address for readability.  with_sorted_ids()
// assigns chip IDs in size-ascending order regardless of declaration order,
