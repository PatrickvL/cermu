...
// =========================================================================
// CIA_REG_SDR delay pipeline constants (per VICE ciacore.c)
...
#define CIA_SDR_SET_SDR_IRQ3    0x0100u  // Countdown to setting CIA_REG_SDR IRQ
...
#define CIA_SDR_SET3        0x00010000u  // Countdown to loading CIA_REG_SDR into shifter
#define CIA_SDR_SET2        0x00020000u
#define CIA_SDR_SET1        0x00040000u
#define CIA_SDR_SET0        0x00080000u  // Action: load CIA_REG_SDR
...
void mos6526_t::reset() {
    ...
    regs_[TIMER_OFFSET + CIA_REG_TA_LO] = 0xFF;
    regs_[TIMER_OFFSET + CIA_REG_TA_HI] = 0xFF;
    regs_[TIMER_OFFSET + CIA_REG_TB_LO] = 0xFF;
    regs_[TIMER_OFFSET + CIA_REG_TB_HI] = 0xFF;
    // Timer counters are also set to all ones after reset
    // (loaded from latches since control registers are 0 = stopped)
    regs_[CIA_REG_TA_LO] = 0xFF;
    regs_[CIA_REG_TA_HI] = 0xFF;
    regs_[CIA_REG_TB_LO] = 0xFF;
    regs_[CIA_REG_TB_HI] = 0xFF;
    ...
    // Already reset above in the grouped CIA_REG_SDR state initialization
...
// INTERRUPT CONTROL REGISTER (CIA_REG_ICR) handling
...
uint8_t mos6526_t::read_and_clear_interrupt_control_register() {
    ...
    uint8_t v = regs_[CIA_REG_ICR];
    
    // "interrupt can be prevented by reading the CIA_REG_ICR at the time of the underflow."
    regs_[CIA_REG_ICR] = 0;
    
    // Timer B Bug: Remember CIA_REG_ICR reads to block Timer B interrupts this cycle
...
void mos6526_t::check_interrupt_mask() {
    ...
    if (masked_interrupts > 0) {
        ...
        // No need for delayed_irq flag - the interrupt persists until CIA_REG_ICR is read.
        regs_[CIA_REG_ICR] |= ICR_IRQ;
    }
    // Per CIA datasheet: "Clearing the bit in the IMR may not clear the interrupt."
    // ICR_IRQ is ONLY cleared by reading the CIA_REG_ICR register (read_and_clear_interrupt_control_register).
    // Once the interrupt flip-flop has been set, changing the IMR has no effect on ICR_IRQ.
}

void mos6526_t::write_interrupt_control_register(uint32_t v) {
    ...
    // "When a condition in the CIA_REG_ICR is true, setting the corresponding bit in the IMR must also set the interrupt."
...
void mos6526_t::write_data_direction_port(uint32_t p, uint8_t v) { // p:A or B
    // Delegate to generic io_port which handles:
    //   - output→input transitions: pull up pins (passive pull-ups)
    //   - input→output transitions: drive CIA_REG_PRA/CIA_REG_PRB value onto pins
...
void mos6526_t::update_output_port(uint32_t p, uint8_t v) { // p:A or B
    // Drive port pin values through output mask
    // For port A: mask = CIA_REG_DDRA (standard)
    // For port B: mask = IDDRB (CIA_REG_DDRB with PBON forced outputs) — see update_internal_data_direction_port_b()
    auto& port = (p == A) ? port_a : port_b;
    ...
    // banking needs to be notified on every CIA_REG_PRA write for correct operation
...
void mos6526_t::update_output_port_b(uint8_t v) {
    ...
    // Note: Port B register (regs_[CIA_REG_PRB]) is already set by the write handler
    ...
    if ((regs_[CIA_REG_CRA] & CRA_PBON) > 0) { // PB6 output mode:Timer
        if ((regs_[CIA_REG_CRA] & CRA_OUTMODE) == 0) {
            ...
            uint8_t timer_a_output = ((regs_[CIA_REG_ICR] & ICR_TA) << 6);
    ...
    // "CIA_REG_CRB[..]1 controls the output of TIMER B on PB7"
    if ((regs_[CIA_REG_CRB] & CRB_PBON) > 0) { // PB7 output mode:Timer
        if ((regs_[CIA_REG_CRB] & CRB_OUTMODE) == 0) {
            // Pulse mode: Output HIGH for one cycle on underflow (ICR_TB set)
            // Will be cleared in next cycle by the pulse clear logic in tick()
            uint8_t timer_b_output = ((regs_[CIA_REG_ICR] & ICR_TB) << 6);
...
uint8_t mos6526_t::read_port_data(uint32_t p) { // p:A or B
    ...
    // For Port A, use regular CIA_REG_DDRA
    uint8_t output_mask = (p == A) ? regs_[CIA_REG_DDRA] : regs_[IDDRB_OFFSET];
    ...
    // PBON makes PB6/PB7 show the timer output (pulse or toggle), not regs_[CIA_REG_PRB].
    // Compute directly from current CIA_REG_ICR/toggle state to avoid stale port_b_value.
    if (p == B) {
        uint8_t pbon_mask = 0;
        uint8_t pbon_value = 0;
        if (regs_[CIA_REG_CRA] & CRA_PBON) {
            pbon_mask |= PB6_MASK;
            if ((regs_[CIA_REG_CRA] & CRA_OUTMODE) == 0) {
                // Pulse mode: PB6 HIGH for exactly one phi2 cycle on timer A underflow.
                // Use timer_underflowed (one-cycle flag cleared each tick), not CIA_REG_ICR
        ...
        return (port_value & ~output_mask)         // Input bits (from pin/callback)
             | (regs_[CIA_REG_PRB] & normal_mask)       // Normal output bits (from register)
             | (pbon_value & pbon_mask);            // PBON bits (timer output, live)
    }
    
    // Return combination: input bits from port_value, output bits from register
    return (port_value & ~output_mask) | (regs_[CIA_REG_PRA + p] & output_mask);
}
...
void mos6526_t::update_internal_data_direction_port_b(uint8_t port_b_output_mask) {
    ...
    // for TIMER B). This function overrides the CIA_REG_DDRB
    // control bit and forces the appropriate PB line to an
    // output."
    if ((regs_[CIA_REG_CRA] & CRA_PBON) > 0)
        port_b_output_mask = port_b_output_mask | PB6_MASK;

    // "CIA_REG_CRB[..]1 controls the output of TIMER B on PB7"
    if ((regs_[CIA_REG_CRB] & CRB_PBON) > 0)
        // Override PB7 when CIA_REG_CRB has PBON flag set
...
void mos6526_t::reload_timer(uint32_t t) { // t:A or B
    uint32_t i = t * 2; // Turn A or B into CIA_REG_TA_LO / CIA_REG_TB_LO offsets
    regs_[CIA_REG_TA_LO + i] = regs_[TIMER_OFFSET + CIA_REG_TA_LO + i];
    regs_[CIA_REG_TA_HI + i] = regs_[TIMER_OFFSET + CIA_REG_TA_HI + i];
    timer_counter_[t] = (regs_[CIA_REG_TA_HI + i] << 8) | regs_[CIA_REG_TA_LO + i];
}
...
void mos6526_t::check_reload_timer(uint32_t t) { // t:A or B
    ...
    if ((regs_[CIA_REG_CRA + t] & CR_START) == 0)
        reload_timer(t);
}

void mos6526_t::decrease_timer(uint32_t t) { // t:A or B
    ...
    if (timer == 0x0000 && count_active) {
        ...
        // Set CIA_REG_ICR flag, but for Timer B check the "Timer B Bug" first
        if (t == A || !icr_read_this_cycle) {
            regs_[CIA_REG_ICR] |= (uint8_t)(ICR_TA + t);
        }
        reload_timer(t);
        ...
        if (t == A && (regs_[CIA_REG_CRA] & CRA_SPMODE)) {
        ...
        if ((regs_[CIA_REG_CRA + t] & CR_RUNMODE) != 0) {
            regs_[CIA_REG_CRA + t] &= ~CR_START;
        }
    }
}

// CONTROL REGISTER (CIA_REG_CRA/CIA_REG_CRB) handling

void mos6526_t::write_control_register(uint32_t c, uint8_t v) { // c:A or B
    uint8_t old_crx = regs_[CIA_REG_CRA + c]; // c=B:CIA_REG_CRB
    
    // CIA_REG_CRA write logging disabled for performance
    ...
    // NOTE: Do NOT clear CIA_REG_ICR bits when manually stopping a timer.
    // Per CIA6526.txt: "Only reading the CIA_REG_ICR will clear it."
    // The CIA_REG_ICR bits represent interrupt conditions that occurred and must persist
    // until software explicitly reads the CIA_REG_ICR register to acknowledge them.
    ...
    if ((v & CR_START) != 0 && (old_crx & CR_START) == 0) {
        ...
        //   2. Force LOAD bit (bit 4 of CIA_REG_CRA/CIA_REG_CRB)
    ...
    if (old_pbon > new_pbon) {
        ...
        // which expects 0x3F to restore to 0xFF once the CIA_REG_CRA/CIA_REG_CRB PBON bits are cleared.
        port_b_value |= ((c == A) ? PB6_MASK : PB7_MASK);
    }

    if (old_pbon != new_pbon)
        update_internal_data_direction_port_b(regs_[CIA_REG_DDRB]);
}
...
uint8_t mos6526_t::latch_read_tod_hr() {
    ...
    regs_[CLOCK_OFFSET + CIA_REG_TOD_10THS] = regs_[CIA_REG_TOD_10THS];
    regs_[CLOCK_OFFSET + CIA_REG_TOD_SEC] = regs_[CIA_REG_TOD_SEC];
    regs_[CLOCK_OFFSET + CIA_REG_TOD_MIN] = regs_[CIA_REG_TOD_MIN];
    return regs_[CLOCK_OFFSET + CIA_REG_TOD_HR] = regs_[CIA_REG_TOD_HR];
}

uint8_t mos6526_t::unlatch_read_tod_10ths() {
    read_tod_delta = 0;
    return regs_[CLOCK_OFFSET + CIA_REG_TOD_10THS];
}
...
void mos6526_t::check_alarm_interrupt() {
    ...
    bool alarm_active = (
        regs_[CIA_REG_TOD_10THS] == regs_[ALARM_OFFSET + CIA_REG_TOD_10THS] &&
        regs_[CIA_REG_TOD_SEC] == regs_[ALARM_OFFSET + CIA_REG_TOD_SEC] &&
        regs_[CIA_REG_TOD_MIN] == regs_[ALARM_OFFSET + CIA_REG_TOD_MIN] &&
        regs_[CIA_REG_TOD_HR] == regs_[ALARM_OFFSET + CIA_REG_TOD_HR]
    );

    // Only set interrupt flag on rising edge (alarm goes from false to true)
    if (alarm_active && !prev_alarm_state) {
        regs_[CIA_REG_ICR] |= ICR_ALRM;
    }
...
uint8_t mos6526_t::bcd_inc(uint32_t r) { // r:CIA_REG_TOD_SEC,CIA_REG_TOD_MIN or CIA_REG_TOD_HR
...
void mos6526_t::increase_tod_and_check_alarm() {
    ...
    if (tod_cycles++ < cycles_tod[(regs_[CIA_REG_CRA] & CRA_TODIN) >> 7])
        return;

    tod_cycles = 0;

    // The TOD pin receives 50Hz or 60Hz from the power supply.
    // CIA_REG_TOD_10THS must increment at 10Hz, so we divide:
    //   50Hz / 5 = 10Hz, or 60Hz / 6 = 10Hz
    // CRA_TODIN selects the expected power frequency (0=60Hz, 1=50Hz)
    int divider = (regs_[CIA_REG_CRA] & CRA_TODIN) ? 5 : 6;
    if (++tod_tick_counter < divider)
        return;

    tod_tick_counter = 0;

    // Increment CIA_REG_TOD_10THS first, THEN check alarm (per VICE behavior)
    if (++regs_[CIA_REG_TOD_10THS] <= 9) {
        check_alarm_interrupt();
        return;
    }

    regs_[CIA_REG_TOD_10THS] = 0;
    ...
    if (bcd_inc(CIA_REG_TOD_SEC) <= 0x59) { // Note the BCD encoding!
        check_alarm_interrupt();
        return;
    }

    regs_[CIA_REG_TOD_SEC] = 0;
    if (bcd_inc(CIA_REG_TOD_MIN) <= 0x59) { // Note the BCD encoding!
        check_alarm_interrupt();
        return;
    }

    regs_[CIA_REG_TOD_MIN] = 0;
    ...
    uint8_t hr_new = bcd_inc(CIA_REG_TOD_HR);
    ...
    regs_[CIA_REG_TOD_HR] = (uint8_t)(hr_PM | hr_HR);
    check_alarm_interrupt();
}

// SERIAL DATA REGISTER (CIA_REG_SDR) handling

void mos6526_t::write_serial_data_register(uint8_t v) {
    regs_[CIA_REG_SDR] = v;
    // In output mode: load data into shift pipeline
    if (regs_[CIA_REG_CRA] & CRA_SPMODE) {
        if (sr_bits == 0) {
...
/**
 * Process the CIA_REG_SDR delay pipeline — called once per tick.
 ...
 */
void mos6526_t::process_sdr_pipeline() {
    // SET0: Load CIA_REG_SDR value into the 16-bit shifter
    if (sdr_delay & CIA_SDR_SET0) {
        if (sr_bits == 0) {
            sr_bits = 16;
            shifter = (uint16_t)regs_[CIA_REG_SDR] << 1;
        } else if (sr_bits == 1) {
            // Mid-completion: append new byte
            shifter |= regs_[CIA_REG_SDR];
            sr_bits = 17;
        } else {
    ...
    // SET_SDR_IRQ0: Set the CIA_REG_SDR interrupt flag (2 cycles after byte complete)
    if (sdr_delay & CIA_SDR_SET_SDR_IRQ0) {
        regs_[CIA_REG_ICR] |= ICR_SP;
    }
...
bus_state_t mos6526_t::registers_read(void* context, bus_state_t bus_state) {
    ...
    switch (r) {
        ...
        case CIA_REG_PRB:
            ...
            break;
        case CIA_REG_DDRA:
            BUS_SET_DATA(bus_state, cia->regs_[CIA_REG_DDRA]);
            break;
        case CIA_REG_DDRB:
            // Note : Assume this always excludes the optional PBON output mask? (If not, use IDDRB!)
            BUS_SET_DATA(bus_state, cia->regs_[CIA_REG_DDRB]);
            break;
        ...
        case CIA_REG_TB_HI:
            ...
            break;
        // Read TOD registers
        case CIA_REG_TOD_10THS: {
            uint8_t tod_value = (cia->read_tod_delta > 0) ? cia->unlatch_read_tod_10ths() : cia->regs_[CIA_REG_TOD_10THS];
            BUS_SET_DATA(bus_state, tod_value);
            break;
        }
        case CIA_REG_TOD_SEC:
            BUS_SET_DATA(bus_state, cia->regs_[cia->read_tod_delta + CIA_REG_TOD_SEC]);
            break;
        case CIA_REG_TOD_MIN:
            BUS_SET_DATA(bus_state, cia->regs_[cia->read_tod_delta + CIA_REG_TOD_MIN]);
            break;
        case CIA_REG_TOD_HR: {
            uint8_t tod_hr_value = (cia->read_tod_delta > 0) ? cia->regs_[CLOCK_OFFSET + CIA_REG_TOD_HR] : cia->latch_read_tod_hr();
            BUS_SET_DATA(bus_state, tod_hr_value);
            break;
        }
        // Read control registers
        case CIA_REG_SDR:
            BUS_SET_DATA(bus_state, cia->regs_[CIA_REG_SDR]);
            break;
        case CIA_REG_ICR: {
        ...
        case CIA_REG_CRA: {
            uint8_t cra_value = cia->regs_[CIA_REG_CRA];
        ...
        case CIA_REG_CRB: {
            uint8_t crb_value = cia->regs_[CIA_REG_CRB];
            // Mask out LOAD bit - it always reads as 0 (strobe bit)
            crb_value &= ~CR_LOAD;
...
bus_state_t mos6526_t::registers_write(void* context, bus_state_t bus_state) {
    mos6526_t* cia = (mos6526_t*)context;
    ...
    switch (r) {
        ...
        case CIA_REG_PRA:
            ...
            break;
        case CIA_REG_PRB:
            cia->regs_[CIA_REG_PRB] = value;
            cia->update_output_port_b(value);
            break;
        case CIA_REG_DDRA:
            cia->write_data_direction_port(A, value);
            // CRITICAL: ALWAYS trigger callback when CIA_REG_DDRA changes
        ...
        case CIA_REG_DDRB:
            ...
            cia->update_output_port_b(cia->regs_[CIA_REG_PRB]);
            break;
        // Write timer latches
        case CIA_REG_TA_LO:
            cia->regs_[TIMER_OFFSET + CIA_REG_TA_LO] = value;
            ...
            break;
        case CIA_REG_TA_HI:
            cia->regs_[TIMER_OFFSET + CIA_REG_TA_HI] = value;
            // Writing to HI byte while stopped triggers reload (latch → counter)
            cia->check_reload_timer(A);
            break;
        case CIA_REG_TB_LO:
            cia->regs_[TIMER_OFFSET + CIA_REG_TB_LO] = value;
            // Per CIA6526 datasheet: Writing to timer registers updates LATCH only
            break;
        case CIA_REG_TB_HI:
            cia->regs_[TIMER_OFFSET + CIA_REG_TB_HI] = value;
            // Writing to HI byte while stopped triggers reload (latch → counter)
            cia->check_reload_timer(B);
            break;
        // Write TOD registers / ALARM latches
        case CIA_REG_TOD_10THS:
            cia->regs_[cia->write_tod_delta + CIA_REG_TOD_10THS] = value;
            ...
            break;
        case CIA_REG_TOD_SEC:
            cia->regs_[cia->write_tod_delta + CIA_REG_TOD_SEC] = value;
            break;
        case CIA_REG_TOD_MIN:
            cia->regs_[cia->write_tod_delta + CIA_REG_TOD_MIN] = value;
            break;
        case CIA_REG_TOD_HR:
            cia->regs_[cia->write_tod_delta + CIA_REG_TOD_HR] = cia->write_tod_hr(value);
 ...
 *   2. Interrupt generation (set CIA_REG_ICR flags, update IRQ line)
 ...
 */
bus_state_t mos6526_t::tick_phi2(bus_state_t bus_state) {
    ...
    // Serial I/O:
    // - Output mode (SPMODE=1): Handled by CIA_REG_SDR delay pipeline (process_sdr_pipeline).
    ...
    if ((regs_[CIA_REG_CRA] & CRA_SPMODE) == 0) {
        ...
        if (cnt_is_positive_edge) {
            ...
            if (sr_bits == 0) {
                // Byte complete: dump into CIA_REG_SDR and generate interrupt
                regs_[CIA_REG_SDR] = shifter & 0xFF;
                regs_[CIA_REG_ICR] |= ICR_SP;
            }
        }
    }
    
    // Process CIA_REG_SDR output delay pipeline (runs every tick)
    if (regs_[CIA_REG_CRA] & CRA_SPMODE) {
    ...
    // Timer A INMODE: bit 5 of CIA_REG_CRA (0=PHI2, 1=CNT)
    bool ta_active = false;
    if ((regs_[CIA_REG_CRA] & CRA_INMODE) == 0) {
        ta_active = true;  // PHI2 mode: always active
    } else {
        ta_active = cnt_is_positive_edge;  // CNT mode: active on positive edge
    }
    if (ta_active && (regs_[CIA_REG_CRA] & CRA_START)) {
    ...
    // Timer B INMODE: bits 5-6 of CIA_REG_CRB (00=PHI2, 01=CNT, 10=Timer A, 11=Timer A+CNT)
    bool tb_active = false;
    uint8_t crb_inmode = regs_[CIA_REG_CRB] & CRB_INMODE;
    switch (crb_inmode) {
        case 0x00:  // PHI2
            tb_active = true;
            break;
    ...
    if (tb_active && (regs_[CIA_REG_CRB] & CRB_START)) {
    ...
    if ((regs_[CIA_REG_CRA] & (CR_START | CR_RUNMODE)) == (CR_START | CR_RUNMODE)) {
        delay_line.Feed(oneshot_a_pipe);
    }
    if ((regs_[CIA_REG_CRB] & (CR_START | CR_RUNMODE)) == (CR_START | CR_RUNMODE)) {
 ...
 */
bus_state_t mos6526_t::tick_phi1(bus_state_t bus_state) {
