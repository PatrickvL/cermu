...
std::vector<PinSignalState> pia6820_t::get_layout_pin_states(ChipLayout& layout) {
    return build_pin_states(layout, bus_snapshot_, [this](auto& ps) {
        ...
        for (int i = 0; i < 8; i++) {
            bool is_output = (port_a_direction >> i) & 1;
            bool data_val  = (port_a_data >> i) & 1;
        ...
        for (int i = 0; i < 8; i++) {
            bool is_output = (port_b_direction >> i) & 1;
            bool data_val  = (port_b_data >> i) & 1;
        ...
        bool cb2_is_output = (port_b_control >> 5) & 1;
        ps[18].signal_level    = cb2_state;
        ...
        bool irqa_asserted = irq_a1 || irq_a2;
