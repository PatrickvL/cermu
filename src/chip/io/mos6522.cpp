...
void mos6522_t::set_port_a_read_callback(uint8_t (*callback)(void*, uint8_t), void* context) {
    ...
    port_a_read_context = context;
}

void mos6522_t::set_port_b_read_callback(uint8_t (*callback)(void*, uint8_t), void* context) {
...
bus_state_t mos6522_t::registers_read(bus_state_t bus_state) {
...
bus_state_t mos6522_t::registers_write(bus_state_t bus_state) {
    ...
    switch (reg) {
        ...
        case MOS6522_T1CH:
            ...
            timer1_running = true;
            ifr &= ~MOS6522_IFR_T1;  // Clear T1 interrupt flag
        ...
        case MOS6522_T2CH: // 0x09 - T2 Counter High (write starts timer)
            ...
            timer2_counter = timer2_latch;
            timer2_running = true;
            ifr &= ~MOS6522_IFR_T2;  // Clear T2 interrupt flag
        ...
        case MOS6522_IER:
            ...
            if (value & 0x80) {
                ier |= (value & 0x7F);   // Set specified bits
            } else {
...
bus_state_t mos6522_t::tick(bus_state_t bus_state) {
    ...
    if (timer1_running) {
        ...
        uint16_t old_counter = timer1_counter;
        timer1_counter--;
    ...
    if (timer2_running && !t2_pulse_count_mode) {
        // Decrement counter first
        uint16_t old_counter = timer2_counter;
        timer2_counter--;
