...
//   5. mask       — address line mask the chip decodes.
//                   For MemoryChipBase-derived types: mask+1 becomes the
//                   flat-memory buffer size (e.g. 0x1FFF → 8192 bytes).
//                   For all other types: stored as addr_mask in ChipSlot
//                   (e.g. 0x3F → 64 register addresses, MMIO-only).
...
//
// Buffer vs MMIO distinction is derived from the chip type:
//   MemoryChipBase subclasses → buffer chip (flat-memory allocation)
//   Everything else           → MMIO-only (register handlers)
...
//
// Buffer/MMIO routing is type-derived:
//   MemoryChipBase subclass → size_bytes = mask+1,  addr_mask = 0
//   Otherwise                → size_bytes = 0,       addr_mask = mask
//
#define CERMU_CHIP_VISITOR_MANIFEST_ROW(ctx, type, chip, base, mask, overlay, label, info_label, rom_files) \
    ChipSlot{base,                                                                                         \
...
// Size is derived as mask+1 (valid for both buffer and MMIO chips).
//
#define CERMU_CHIP_VISITOR_INFO_ROW(ctx, type, chip, base, mask, overlay, label, info_label, rom_files) \
    {base, (size_t)(mask) + 1, info_label},
...
//
#define CERMU_CHIP_VISITOR_DECLARE_POINTER(ctx, type, chip, base, mask, overlay, label, info_label, rom_files) \
    type* chip = nullptr;
...
//
#define CERMU_CHIP_VISITOR_COUNT_ONE(ctx, type, chip, base, mask, overlay, label, info_label, rom_files) +1
