...
int test_tia_hmove_blanking(harness_t* h) {
    ...
    h->tia.vblank_active = false;
    h->tia.visible_row = 0;
    h->tia.colup0 = 0x1A;   // Non-zero color
    h->tia.colubk = 0x00;
    h->tia.grp0 = 0xFF;
    h->tia.pos_p0 = 0;
    h->tia.nusiz0 = 0;
    ...
    uint32_t bg_color = h->tia.palette_rgba_[0];
    uint32_t p0_color = h->tia.palette_rgba_[(h->tia.colup0 >> 1) & 0x7F];
...
int test_tia_score_mode_priority_interaction(harness_t* h) {
    ...
    h->tia.pos_p0 = 20;
    h->tia.nusiz0 = 0;
    h->tia.pf0 = 0xFF;   // All PF0 bits set
    ...
    uint32_t colup0_rgba = h->tia.palette_rgba_[(h->tia.colup0 >> 1) & 0x7F];
    ...
    uint32_t colup1_rgba = h->tia.palette_rgba_[(h->tia.colup1 >> 1) & 0x7F];
...
int test_tia_missile_copies(harness_t* h) {
    ...
    h->tia.resmp0 = false;
    h->tia.pos_m0 = 40;
    h->tia.colup0 = 0x1A;   // Non-zero color for missile 0
    h->tia.colubk = 0x00;
    h->tia.grp0 = 0;        // No player graphics
    ...
    uint32_t m0_color = h->tia.palette_rgba_[(h->tia.colup0 >> 1) & 0x7F];
    uint32_t bg_color = h->tia.palette_rgba_[0];
...
int test_tia_vblank_transition_mid_scanline(harness_t* h) {
    ...
    A26_ASSERT_TRUE(h, "VBL_MID", !h->tia.vblank_active,
...
int test_tia_hmove_motion_sign_convention(harness_t* h) {
    ...
    A26_ASSERT_EQ(h, "HM_SIGN", h->tia.hm_p0, 7,
    ...
    A26_ASSERT_EQ(h, "HM_SIGN", h->tia.hm_p0, -8,
    ...
    A26_ASSERT_EQ(h, "HM_SIGN", h->tia.hm_p0, -1,
...
int test_tia_playfield_score_mode_with_priority(harness_t* h) {
    ...
    h->tia.ctrlpf = 0x06;
    h->tia.colup0 = 0x2A;   // Player 0 color
    h->tia.colup1 = 0x4A;   // Player 1 color
    h->tia.colupf = 0x6A;   // PF color (should NOT be used when score mode active)
    h->tia.colubk = 0x00;
...
int test_tia_player_copy_positions_exact(harness_t* h) {
    ...
    h->tia.colup0 = 0x1A;
    h->tia.colubk = 0x00;
    h->tia.grp0 = 0x80;  // Single pixel (bit 7 = leftmost)
    h->tia.pos_p0 = 10;
    h->tia.refp0 = false;
...
int test_tia_grp_vdel_cross_latch_sequence(harness_t* h) {
    ...
    A26_ASSERT_EQ(h, "GRP_XLATCH", h->tia.grp0, 0xFF,
                  "GRP0 written to $FF");
    ...
    A26_ASSERT_EQ(h, "GRP_XLATCH", h->tia.grp1, 0xCC,
                  "GRP1 written to $CC");
...
int test_tia_enam_bit1_only(harness_t* h) {
    ...
    A26_ASSERT_TRUE(h, "ENAM_BIT", !h->tia.enam0,
    ...
    A26_ASSERT_TRUE(h, "ENAM_BIT", !h->tia.enam1,
    ...
    A26_ASSERT_TRUE(h, "ENAM_BIT", !h->tia.enabl,
...
int test_tia_late_hmove_no_blanking(harness_t* h) {
    ...
    h->tia.grp0 = 0xFF;
    h->tia.pos_p0 = 0;
    h->tia.nusiz0 = 0;  // Single copy

    // Set motion registers
    h->tia.hm_p0 = -2;  // Move right 2
...
int test_tia_write_address_mirroring(harness_t* h) {
    ...
    A26_ASSERT_EQ(h, "WR_MIR", (int)h->tia.colubk, 0x4E,
                  "COLUBK via $09");

    // Write COLUBK via mirror at $09 + $40 = $49
    h->tia.write(0x49, 0x82);
    A26_ASSERT_EQ(h, "WR_MIR", (int)h->tia.colubk, 0x82,
                  "COLUBK via mirror $49");

    // Write GRP0 via $1B and mirror at $5B
    h->tia.write(TIA_GRP0, 0xAA);
    A26_ASSERT_EQ(h, "WR_MIR", (int)h->tia.grp0, 0xAA,
                  "GRP0 via $1B");
    h->tia.write(0x5B, 0x55);
    A26_ASSERT_EQ(h, "WR_MIR", (int)h->tia.grp0, 0x55,
                  "GRP0 via mirror $5B");
...
int test_tia_playfield_all_40_pixels(harness_t* h) {
    ...
    h->tia.pf1 = 0xAA;
    h->tia.pf2 = 0x55;
    h->tia.ctrlpf = 0;  // No reflect, no score, no priority
...
int test_tia_color_register_bit0_ignored(harness_t* h) {
    ...
    // Write odd value to COLUP0
    h->tia.write(TIA_COLUP0, 0x1B);  // Bit 0 = 1
    A26_ASSERT_EQ(h, "COL_B0", (int)h->tia.colup0, 0x1A,
                  "COLUP0 masks bit 0: $1B → $1A");

    h->tia.write(TIA_COLUP1, 0xFF);
    A26_ASSERT_EQ(h, "COL_B0", (int)h->tia.colup1, 0xFE,
                  "COLUP1 masks bit 0: $FF → $FE");

    h->tia.write(TIA_COLUPF, 0x01);
    A26_ASSERT_EQ(h, "COL_B0", (int)h->tia.colupf, 0x00,
                  "COLUPF masks bit 0: $01 → $00");

    h->tia.write(TIA_COLUBK, 0x83);
    A26_ASSERT_EQ(h, "COL_B0", (int)h->tia.colubk, 0x82,
                  "COLUBK masks bit 0: $83 → $82");
...
int test_tia_vblank_dump_paddle_capacitors(harness_t* h) {
    ...
    A26_ASSERT_EQ(h, "PADDLE", (int)(inpt0_val & 0x80), (int)(h->tia.inpt0 ? 0x80 : 0x00),
...
int test_tia_collision_all_15_pairs(harness_t* h) {
    ...
    auto test_pair = [&](const char* label, uint16_t expected_flag,
        ...
        h->tia.grp0 = set_p0 ? 0xFF : 0x00;
        h->tia.grp1 = set_p1 ? 0xFF : 0x00;
        h->tia.enam0 = set_m0;
        h->tia.enam1 = set_m1;
        h->tia.enabl = set_bl;
